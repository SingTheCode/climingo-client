...
| `architecture.md` | 아키텍처 규칙 | 핵심 원칙, 디렉토리 구조, 참고 자료 |
| `architecture-rules.md` | 세부 규칙 | Rule 1-7 세부 규칙 |
| `architecture-domain-isolation.md` | 도메인 격리 | 도메인 간 의존성 관리, 실전 패턴 |
...
- **추가**: 관련 섹션에 내용 추가 (파일 길이 < 500줄)
- **분리**: 파일 길이 > 500줄이면 관련 내용을 새 파일로 추출
- **생성**: 기존 파일과 관련 없는 새로운 주제면 새 파일 생성

### 3. 파일 분리 기준

파일이 500줄을 초과하면 다음 기준으로 분리:
