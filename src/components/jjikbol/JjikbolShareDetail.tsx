...
interface JjikbolShareData {
  ...
  gym: {
    gymId: number; // API에서 number로 반환됨
    gymName: string;
  };
  level: {
    levelId: number; // API에서 number로 반환됨
    colorNameKo: string;
    colorNameEn: string;
  };
  isEditable: boolean;
...
export default function JjikbolShareDetail() {
  const params = useParams();
  const jjikbolId = params?.jjikbolId as string;

  const { data, isLoading, isError } = useGetJjikbolDetailQuery(jjikbolId);
  const { shareToNative, saveAsImage } = useJjikbol();

  const handleShareClick = () => {
    if (data) {
      // JjikbolData 인터페이스에 맞게 데이터 변환
      const jjikbolData = {
        jjikbol: data.jjikbol,
        memberInfo: {
  ...
  return (
    <div
      id="jjikbol-share-container"
      data-testid="jjikbol-share-container"
    ...
    >
      ...
      {/* 설명 텍스트 영역 */}
      <DescriptionSection description={data.jjikbol.description} />
...
// 메인 이미지 섹션 컴포넌트
function MainImageSection({ data }: { data: JjikbolShareData }) {
  return (
    <div className="relative w-full h-[674px] bg-[#292929] overflow-hidden">
      {/* 배경 이미지 */}
      <div
        className="absolute inset-0 bg-cover bg-center"
        data-testid="jjikbol-problem-image"
        style={{
...
// 사용자 정보 섹션 컴포넌트
function UserInfoSection({ data }: { data: JjikbolShareData }) {
  return (
